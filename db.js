...
function createSchema() {
  ...
  var cols = ['monthly_rent TEXT', 'security_deposit TEXT', 'cleaning_fee TEXT', 'unit_address TEXT'];
  for (var i = 0; i < cols.length; i++) {
    try { db.run('ALTER TABLE applications ADD COLUMN ' + cols[i]); } catch(e) { /* already exists */ }
  }
...
function seedAdmin() {
  ...
  if (!existing) {
    ...
    run('INSERT INTO admins (email, password_hash, name) VALUES (?, ?, ?)', [adminEmail, hash, 'GreyHaven Admin']);
...
var q = {
  insertApplication: function(app) {
    ...
    var id = run('INSERT INTO applications (' + cols.join(', ') + ') VALUES (' + cols.map(function(){return '?';}).join(', ') + ')', vals);
    if (id) return this.getApplicationById(id);
    return get('SELECT * FROM applications WHERE ref_number = ?', [app.ref_number]);
  },
  getAllApplications:          function() { return all('SELECT * FROM applications ORDER BY submitted_at DESC'); },
  getApplicationById:          function(id) { return get('SELECT * FROM applications WHERE id = ?', [id]); },
  getApplicationByRef:         function(ref) { return get('SELECT * FROM applications WHERE ref_number = ?', [ref]); },
  getApplicationByLeaseToken:  function(token) { return get('SELECT * FROM applications WHERE lease_token = ?', [token]); },
  updateStatus: function(p) {
    run('UPDATE applications SET status=?, admin_notes=?, lease_token=?, reviewed_at=datetime("now"), reviewed_by=?, monthly_rent=?, security_deposit=?, cleaning_fee=?, unit_address=? WHERE id=?',
      [p.status, p.admin_notes, p.lease_token, p.reviewed_by, p.monthly_rent||null, p.security_deposit||null, p.cleaning_fee||null, p.unit_address||null, p.id]);
  },
  getStats: function() {
    return get('SELECT COUNT(*) as total, SUM(CASE WHEN status="pending" THEN 1 ELSE 0 END) as pending, SUM(CASE WHEN status="approved" THEN 1 ELSE 0 END) as approved, SUM(CASE WHEN status="rejected" THEN 1 ELSE 0 END) as rejected FROM applications');
  },
  getAdminByEmail: function(email) { return get('SELECT * FROM admins WHERE email = ?', [email]); },
  logEmail: function(app_id, type, to_email, success) {
    run('INSERT INTO email_log (app_id, type, to_email, success) VALUES (?,?,?,?)', [app_id, type, to_email, success]);
  },
  getEmailLog: function(app_id) { return all('SELECT * FROM email_log WHERE app_id=? ORDER BY sent_at DESC', [app_id]); },
};
