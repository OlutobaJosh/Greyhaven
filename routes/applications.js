...
router.get('/:id', requireAuth, (req, res) => {
  try {
    ...
    // Include payment confirmation fields
...
router.post('/lease/:token/confirm', async (req, res) => {
  try {
    ...
    // Mark lease as signed in DB
    q.markLeaseSigned(app.id, new Date().toISOString());
...
// PUT /api/applications/:id/confirm-payment — Admin confirms payment + issues booking receipt
router.put('/:id/confirm-payment', requireAuth, async (req, res) => {
  try {
    ...
    q.confirmPayment(app.id, req.admin.email);

    // Build booking receipt email
    const confirmedApp = q.getApplicationById(app.id);
    ...
    const receiptHtml = '<!DOCTYPE html><html><head><meta charset="UTF-8"/><style>' +
      ...
      '.logo{color:#fff;font-size:24px;font-weight:700;letter-spacing:.02em;}' +
      ...
      '.receipt-num strong{display:block;font-size:20px;color:#0f1e32;letter-spacing:.05em;}' +
    ...
    q.markReceiptSent(app.id);
    q.logEmail(app.id, 'booking_receipt', app.email, 1);
